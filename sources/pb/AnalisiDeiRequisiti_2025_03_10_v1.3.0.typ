...
- Navigazione alla dashboard di business (UC33)
...
=== UC43 - Visualizzazione mappa negozi selezionati
*Attore principale*: 
- Amministratore 
...
- *RF1.1*: Il sistema deve supportare autenticazione JWT con username/password via endpoint `/api/token`
- *RF1.2*: I token devono avere scadenza configurabile tramite `JWT_EXPIRATION_S` (default 1 ora)
- *RF1.3*: Le credenziali devono essere validate contro database ClickHouse con hash sicuri
...
- *RF2.1*: Il sistema deve simulare movimenti utenti con `producer.py` su percorsi Milano OSRM
- *RF2.2*: Gli eventi GPS devono essere prodotti in Kafka topic `gps_stream` con SSL
- *RF2.3*: I percorsi devono essere calcolati usando OSRM self-hosted con profilo cycling
- *RF2.4*: Gli eventi devono contenere: user_id, latitude, longitude, timestamp, poi_info
- *RF2.5*: Il producer deve supportare readiness checks per Kafka, ClickHouse e OSRM

==== RF3 - Elaborazione stream e proximity detection
- *RF3.1*: Il sistema deve processare eventi GPS via Bytewax dataflow con Observer pattern
- *RF3.2*: Deve calcolare distanza usando query PostGIS `ST_DWithin` per soglia 200m
...
- *RF4.4*: Deve gestire offerte con Strategy Pattern (Standard/Aggressive/Conservative)
- *RF4.5*: Deve supportare Builder Pattern per creazione offerte complesse
- *RF4.6*: Factory Pattern deve creare offerte tipizzate (Flash/Student/Senior/Category)
...
- *RF5.2*: Deve implementare mappa Leaflet con marker categorizzati per shop types
- *RF5.3*: Deve visualizzare percorso utente come polyline con history
- *RF5.4*: Deve supportare filtri categoria con mapping predefinito
- *RF5.5*: Deve implementare fallback WebSocket→HTTP polling automatico
...
- *RV1.5*: Supporto browser moderni con API JavaScript ES6+
- *RV1.6*: Dipendenze esterne: Leaflet.js, Font Awesome 6.4.0+

==== RV2 - Browser e Compatibilità
- *RV2.1*: Chrome/Chromium ≥ 51 (giugno 2016)
- *RV2.2*: Firefox ≥ 55 (agosto 2017)
- *RV2.3*: Safari ≥ 12.1 (marzo 2019)
- *RV2.4*: Microsoft Edge ≥ 79 (gennaio 2020, solo Chromium)
- *RV2.5*: Safari iOS ≥ 12.2 per dispositivi mobili
- *RV2.6*: Esclusione esplicita: Internet Explorer, Edge Legacy

==== RV3 - Tecnologie Frontend Richieste
- *RV3.1*: WebSocket API per comunicazione real-time
- *RV3.2*: Fetch API per richieste HTTP asincrone
- *RV3.3*: IntersectionObserver API per lazy loading
- *RV3.4*: Web Storage API (sessionStorage/localStorage)
- *RV3.5*: CSS Custom Properties e CSS Grid/Flexbox
- *RV3.6*: JavaScript abilitato obbligatorio

==== RV4 - Geografici
- *RV4.1*: Sistema focalizzato su Milano (bbox configurata)
- *RV4.2*: Coordinate sistema: WGS84 (EPSG:4326)
- *RV4.3*: Percorsi limitati a rete stradale esistente (OSRM)

==== RV5 - Operativi
- *RV5.1*: Ambiente development single-machine
- *RV5.2*: Configurazione tramite variabili ambiente
- *RV5.3*: Logging su stdout per container orchestration
- *RV5.4*: Gestione segreti tramite Docker secrets o env vars
- *RV5.5*: Fallback automatico WebSocket→HTTP polling
- *RV5.6*: Design responsivo con breakpoint mobile (≥320px)

==== RV6 - Requisiti Hardware Minimi
- *RV6.1*: RAM minima: 8GB (raccomandati 16GB)
- *RV6.2*: Connessione Internet per WebSocket real-time
- *RV6.3*: Risoluzione display minima: 320px larghezza

== Requisiti funzionali desiderabili

=== RFD1 - Ottimizzazioni frontend avanzate
- *RFD1.1*: Il sistema dovrebbe implementare lazy loading notifiche con IntersectionObserver
- *RFD1.2*: Dovrebbe supportare tema scuro/chiaro configurabile dall'utente con localStorage
- *RFD1.3*: Dovrebbe fornire local cache frontend per shop areas e notifications
- *RFD1.4*: Dovrebbe limitare e ottimizzare rendering markers (max 100 shops)
- *RFD1.5*: Dovrebbe implementare design responsivo con breakpoint mobile

=== RFD2 - Monitoring e osservabilità avanzate
- *RFD2.1*: Il sistema dovrebbe esporre analisi Prometheus tramite FastAPI instrumentator
- *RFD2.2*: Dovrebbe implementare dashboard Grafana con pannelli assemblati dinamicamente
- *RFD2.3*: Dovrebbe supportare logging strutturato con livelli configurabili
- *RFD2.4*: Dovrebbe includere exporters per database e servizi (Postgres, Redis, ClickHouse)
- *RFD2.5*: Dovrebbe supportare Loki/Promtail per log aggregation
- *RFD2.6*: Dovrebbe implementare cAdvisor per container monitoring
...
- *RFD3.2*: Dovrebbe implementare paginazione avanzata per notifiche e promozioni
- *RFD3.3*: Dovrebbe fornire filtri dinamici Grafana (età, professione, categoria, popolarità)
- *RFD3.4*: Dovrebbe supportare mappe interattive con route visualization
- *RFD3.5*: Dovrebbe implementare shop visit simulation con feedback realtime

=== RFD4 - Performance e scalabilità
- *RFD4.1*: Il sistema dovrebbe implementare connection pooling avanzato
- *RFD4.2*: Dovrebbe supportare cache distribuita con clustering Redis
- *RFD4.3*: Dovrebbe fornire push gateway per metriche batch
- *RFD4.4*: Dovrebbe implementare graceful degradation per service failures

== Requisiti (funzionali) facoltativi

=== RFF1 - Integrazioni esterne
- *RFF1.1*: Il sistema potrebbe integrare API meteo per context-aware messaging
- *RFF1.2*: Potrebbe supportare payment integration per offer redemption

=== RFF2 - AI/ML Enhancement
- *RFF2.1*: Potrebbe utilizzare reinforcement learning per offer optimization

=== RFF3 - Business Intelligence avanzato
- *RFF3.1*: Potrebbe supportare A/B testing per offer strategies
...
#req-table(
  ...
  [RFD3.4 - Mappe interattive route], [UC15, UC27, UC39],
  ...
  [RV6.3 - Risoluzione 320px], [UC5-UC19],
    // UC43
    [RV6.4 - Tracciamento percorsi utenti], [UC43],
)
...
#uc-table(
  ...
  [UC43 - Mappa percorsi utenti], [RV6.4, RFD3.4, RNF2.1, RNF2.2, RV1.1, RV1.2, RV1.3, RV1.4, RV1.5, RV1.6, RV2.1-RV2.6, RV3.1, RV3.2, RV3.5, RV3.6, RV4.1, RV4.2, RV4.3, RV5.1, RV5.2, RV5.3, RV5.6, RV6.1, RV6.2, RV6.3]
)