...
#show: bubble.with(
  ...
  version: "2.0.0",
  version_history: (   
    "2.0.0", "2025-08-12", "Controllo e correzioni finali del documento",
    "Giovanni Battista Matteazzi, Elia Leonetti", 
    "Alessandro Di Pasquale, Nicolò Bovo",
...
==== Specifica Tecnica
La Specifica Tecnica rappresenta un documento di approfondimento che illustra nel dettaglio le scelte architetturali, tecnologiche e implementative adottate per lo sviluppo della piattaforma NearYou#apice("G"). Questo documento fornisce una visione completa delle soluzioni tecniche implementate per soddisfare i requisiti funzionali e non funzionali identificati.

Il documento comprende:

- *Tecnologie utilizzate*: analisi dettagliata di tutti gli strumenti, linguaggi di programmazione, framework e infrastrutture adoperate, con motivazione delle scelte effettuate. Include la descrizione di Docker#apice("G") per la containerizzazione#apice("G"), Python e JavaScript#apice("G") per lo sviluppo, Kafka#apice("G") per il message brokering, Bytewax#apice("G") per lo stream processing#apice("G"), PostgreSQL#apice("G")/PostGIS#apice("G") e ClickHouse#apice("G") per la persistenza dati, e l'integrazione con LLM#apice("G") per la generazione di contenuti personalizzati.

- *Architettura del sistema*: presentazione dell'architettura Lambda adottata, con separazione tra batch layer, speed layer e serving layer. Vengono descritti i flussi di dati, le interconnessioni tra componenti e le strategie di scalabilità#apice("G") implementate.

- *Design patterns*: documentazione dei pattern architetturali utilizzati (Singleton#apice("G"), Factory Pattern#apice("G"), Strategy Pattern#apice("G"), Observer pattern#apice("G")) con esempi di implementazione e motivazioni delle scelte progettuali.

- *Diagrammi delle classi*: rappresentazione UML delle principali classi del sistema, organizzate per domini funzionali, con descrizione di attributi, metodi e relazioni.

- *Tracciabilità dei requisiti*: mappatura completa che dimostra come ogni requisito funzionale sia stato soddisfatto attraverso le soluzioni implementate, con indicazione dello stato di completamento.

La Specifica Tecnica costituisce un riferimento fondamentale per comprendere le decisioni architetturali e per facilitare future attività di manutenzione ed evoluzione del sistema.

==== Manuale Utente
Il Manuale Utente è un documento pratico e operativo che guida gli utilizzatori finali e gli amministratori#apice("G") nell'utilizzo corretto e completo della piattaforma NearYou#apice("G"). È strutturato per fornire istruzioni chiare e dettagliate, accompagnate da esempi visivi e procedure#apice("G") passo-passo.

Il documento include:

- *Requisiti e installazione*: istruzioni complete per l'avvio del sistema attraverso Docker Compose#apice("G"), sia in ambiente Gitpod che in locale. Vengono fornite le procedure#apice("G") per il download della repository, la configurazione dell'ambiente e l'avvio dei servizi.

- *Collegamento ai servizi*: guida dettagliata per accedere alle diverse interfacce esposte dal sistema, con particolare attenzione agli URL generati da Gitpod e alle modalità di forwarding delle porte.

- *Dashboard#apice("G") amministratore#apice("G")*: descrizione completa delle tre dashboard#apice("G") Grafana#apice("G") disponibili:
  - *Dashboard#apice("G") Business*: metriche operative, KPI di conversione, analisi geografiche e trend temporali
  - *Dashboard#apice("G") Analisi Marketing*: segmentazione audience, pattern di movimento utenti e analytics#apice("G") comportamentali
  - *Dashboard#apice("G") Monitoraggio#apice("G") Sistema*: metriche infrastrutturali, performance e resource utilization

- *Dashboard#apice("G") utente*: istruzioni per l'utilizzo dell'interfaccia utente finale, inclusa l'autenticazione, la navigazione della mappa interattiva, la ricezione di notifiche personalizzate e la gestione del profilo.

- *Risoluzione problemi*: sezione dedicata alla diagnosi e risoluzione delle problematiche più comuni, con procedure#apice("G") di troubleshooting per connessioni, performance e configurazioni.

Il Manuale Utente rappresenta la guida essenziale per sfruttare appieno le funzionalità della piattaforma e garantire un'esperienza utente ottimale.
...
//   - *Descrizione*: Esprime il rapporto tra il valore guadagnato e il costo reale. Un valore più elevato corrisponde a una maggiore efficienza.
//   - *Formula*:
//   $ "CPI" = "EV" / "AC" $
//   - *Parametri*:
//     - *EV*: _Earned Value_, ossia il valore guadagnato.
//     - *AC*: _Actual Cost_, ossia il costo reale.
