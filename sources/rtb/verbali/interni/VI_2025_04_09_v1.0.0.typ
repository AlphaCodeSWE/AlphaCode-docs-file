...
#show: bubble.with(
  ...
  version: "1.0.0",
  version_history: (
    "1.0.0", "2025-04-09", "Stesura Verbale", "Giovanni Battista Matteazzi, Manuel Cinnirella",
  ),
...
== Ordine del giorno
 ...
 1. Ricapitolazione dei punti principali emersi durante il meeting con l'azienda Sync Lab#apice("G").
 ...
 3. Definizione degli step operativi da intraprendere a breve termine, tra cui:
  - Studio e utilizzo di Docker#apice("G") per il deployment delle componenti;
  - Progettazione e sviluppo di uno script per la generazione di dati (simulazione GPS#apice("G") e status);
  - Redazione dei documenti di progetto: secondo verbale esterno, quinto verbale interno, glossario, norme di progetto.

= Sintesi dell'incontro
 La riunione è iniziata con un riepilogo dei contenuti emersi nel precedente incontro con l'azienda Sync Lab#apice("G"), in cui si è approfondito il contesto operativo e l'architettura tecnica che sarà alla base del progetto. Nonostante ci fossero più tecnologie disponibili tra cui scegliere per la fase implementativa, onde favorire il corretto flusso dei dati è stato deciso di attenersi a quelle consigliate e già utilizzate dall'azienda; questo ci assicura di operare in un contesto noto ai proponenti. 
 I membri che non avevano preso parte al meeting sono stati inoltre messi al corrente degli spunti operativi offerti da Sync Lab#apice("G"), sia a livello tecnico che organizzativo. 
 Il gruppo ha quindi valutato le competenze necessarie per affrontare le prossime fasi del lavoro, individuando come priorità l'apprendimento del framework#apice("G") Docker#apice("G"), e la stesura di uno script Python che consenta di simulare la generazione di dati GPS#apice("G") e di stato. 
 Parallelamente sono stati individuati i documenti da redigere nel breve termine: verbali (interno ed esterno), glossario tecnico e norme di progetto. La discussione si è chiusa con l'individuazione delle scadenze per ciascuna attività e con l'assegnazione dei compiti sulla base della rotazione settimanale dei ruoli. Quest'ultima è stata effettivamente su base settimanale a partire dal 2025-05-03, prima di allora sono state applicate due rotazioni da tre settimane, ridotte poi a due rotazioni da due settimane, per garantire maggiore stabilità e la possibilità di apprendere meglio il proprio ruolo in quelle che erano le prime fasi del progetto. Si è decisa una riduzione graduale della durata di ogni rotazione in virtù di un adattamento graduale ai nuovi ritmi di lavoro. 

= Decisioni prese

 1. Formazione individuale: ogni membro dovrà approfondire l'utilizzo di Docker#apice("G") e preparare degli script in Python per la simulazione dei dati in vista dello sviluppo dei primi prototipi.
 2. Redazione documenti: è stato stabilito che nel corso della settimana verranno redatti:
  - Il secondo verbale esterno, per documentare l'incontro con Sync Lab#apice("G");
  - Il quinto verbale interno;
  - Il glossario tecnico, che accompagnerà tutta la documentazione futura;
  - Le Norme di Progetto, redatte secondo lo standard ISO/IEC 12207:1995#apice("G").
...
#align(center,
  table(
    ...
    [#link("https://github.com/AlphaCodeSWE/NearYou-Project")[Project]], [Nicolò Bovo, Massimo Chioru ,Elia Leonetti], [Script Python per generazione di dati GPS#apice("G") e di status],
